...
- Last verified command: `bash scripts/verify.sh`, `bash scripts/browser-smoke.sh`, Claude audit worker, and `code-review-graph build --skip-flows && code-review-graph detect-changes` passed after module entitlement and independent portal login smoke coverage
...
- code-review-graph evidence: latest `code-review-graph build --skip-flows && code-review-graph detect-changes` ran; graph built 135 files / 1485 nodes / 16559 edges; risk score 0.55 because smoke helper functions are reported as gaps while the changed product journey is covered by real Chrome.
