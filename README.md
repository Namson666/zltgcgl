...
- Last verified command: `bash scripts/verify.sh`, focused Chrome labor deep smoke, full `bash scripts/browser-smoke.sh`, Claude audit worker, and `code-review-graph build --skip-flows && code-review-graph detect-changes` passed after labor personnel/attendance/salary/risk deep CRUD/upload/download/state-change fixes
...
- code-review-graph evidence: latest `code-review-graph build --skip-flows && code-review-graph detect-changes` ran; graph built 135 files / 1529 nodes / 19019 edges; risk score 0.50 because labor helper functions/components are reported as function-level unit gaps while the changed product journey is covered by real Chrome and backend API assertions.
...
- Browser smoke evidence: `docs/smoke-evidence/playwright-results.json`, legacy screenshots, `docs/smoke-evidence/full-route-*.png`, `docs/smoke-evidence/供应商班组CRUD.png`, `docs/smoke-evidence/劳资工资发放导出.png`, `docs/smoke-evidence/劳资人员考勤工资风控CRUD.png`, `docs/smoke-evidence/模块开通独立登录验收.png`, `docs/smoke-evidence/基础后台CRUD.png`, `docs/smoke-evidence/物资主链路CRUD.png`, `docs/smoke-evidence/财务备用金费用CRUD上传.png`, and `docs/smoke-evidence/财务发票收款盈亏导入导出.png`; latest real Chrome smoke passed 12 tests
